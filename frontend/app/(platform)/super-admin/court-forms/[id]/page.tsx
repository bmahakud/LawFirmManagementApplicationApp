...
export default function SuperAdminCourtFormDetailPage() {
  ...
  useEffect(() => {
    const fetchAndRedirect = async () => {
      try {
        if (fromCase) {
          router.replace(`/super-admin/cases/${fromCase}?tab=Documents&subtab=court_forms&formId=${formId}`);
          return;
        }
        ...
        if (res.ok) {
          ...
          if (caseId) {
            router.replace(`/super-admin/cases/${caseId}?tab=Documents&subtab=court_forms&formId=${formId}`);
            return;
          }
        }
      } catch (err) {
