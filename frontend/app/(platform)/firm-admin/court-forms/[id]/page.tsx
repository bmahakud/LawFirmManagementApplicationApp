...
export default function FirmAdminCourtFormDetailPage() {
  ...
  useEffect(() => {
    const fetchAndRedirect = async () => {
      try {
        if (fromCase) {
          router.replace(`/firm-admin/cases/${fromCase}?tab=Documents&subtab=court_forms&formId=${formId}`);
          return;
        }
        ...
        if (res.ok) {
          ...
          if (caseId) {
            router.replace(`/firm-admin/cases/${caseId}?tab=Documents&subtab=court_forms&formId=${formId}`);
            return;
          }
        }
      } catch (err) {
